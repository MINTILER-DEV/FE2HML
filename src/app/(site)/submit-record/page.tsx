        ...
        {hasMaps ? (
              ...
              <select className="h-11 w-full rounded-2xl border border-white/10 bg-slate-900/80 px-4" id="mapId" name="mapId" required>
                {maps.map((map) => (
                  <option key={map.id} value={map.id}>
                    {map.mapCode} • {map.name} ({map.gameType})
                  </option>
                ))}
              </select>
            </div>
          ...
          <div className="space-y-4">
            <p className="text-slate-300">
              No maps are available yet, so record submissions are temporarily
              closed until staff add the first entries.
            </p>
            <Button asChild variant="ghost">
              <Link href="/admin/maps">Go to map management</Link>
            </Button>
          </div>
        )}
      </Card>
