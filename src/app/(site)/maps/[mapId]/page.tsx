...
import { getMapByCode } from "@/lib/data/site";
...
export default async function MapDetailPage({
  ...
  const data = await getMapByCode(mapId);
      ...
      <section className="section-grid">
        <Card className="p-7">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <Badge>{map.mapCode}</Badge>
            <Badge>{map.difficultyLabel}</Badge>
        ...
        <Card className="p-7">
          <div className="grid gap-5 sm:grid-cols-2">
            {[
              ["Map ID", map.mapCode],
              ["List position", map.placement ?? "Unplaced"],
              ["Difficulty", `${map.difficultyScore.toFixed(2)} ${map.difficultyLabel}`],
          ...
          <div className="mt-5 space-y-4">
            {records.length ? (
              ...
              <div className="rounded-3xl border border-dashed border-white/10 bg-white/[0.03] px-4 py-8 text-center text-slate-400">
                No accepted records for this map yet.
              </div>
            )}
          </div>
        </Card>
